...
if not exist "%WORKSPACE_ROOT%\allocated_ports.env" (
    echo [ERROR] Ports not allocated. Run allocate_ports.cmd first.
    pause
    exit /b
)
for /f "tokens=2 delims==" %%A in ('findstr "VM_PORT" "%WORKSPACE_ROOT%\allocated_ports.env"') do set "VM_PORT=%%A"

:: Debug: Show VM_PORT
echo [DEBUG] VM_PORT: !VM_PORT!
...
adb -H !SERVER_IP! -P !ADB_PORT! devices >"%TEMP%\flutter_devices.txt" 2>&1
if errorlevel 1 (
